...
Apollo Graph Manager's **schema registry** is a central hub for managing your data graph. It enables you to explore your schema's types and fields, track its change history, and improve your production server's security by removing the need for introspection. You can also [register **variants** of your schema](#managing-environments-with-variants) that correspond to different environments, such as staging and production.

Apollo provides two mechanisms for schema registration, one [automatic](#automatic-registration) and one [manual](#manual-registration). We recommend automatic registration for architectures that support it.

> **Do not use both automatic and manual schema registration for the same graph.** Doing so can cause you to register "no-op" schema changes, in which the two mechanisms register schemas that are semantically identical but cosmetically different. For example, automatic registration preserves a schema's comments and directives, whereas manual registration does not.

## Automatic registration

> **Automatic schema registration is not available for graphs that use Apollo Federation.** If you have a federated graph, instead see [Setting up managed federation](./managed-federation/setup/).

### Apollo Server setup

Apollo Server version 2.15.1 and later can register its schema with Graph Manager automatically every time it starts up. To do so:
...
Now every time your server finishes starting up, it registers its schema with Graph Manager.

> **Note:** After starting up, Apollo Server waits a random amount of time between zero and ten seconds before automatically registering its schema. [You can customize this behavior.](#customizing-behavior-in-apollo-server)
...
|---|---|---|
| `engine.overrideReportedSchema` | String | If provided, Apollo Server sends this string value as the schema to register. By default, Apollo Server sends its interpreted `GraphQLSchema` object (the same value it returns from an introspection request) as an SDL string. This default behavior is appropriate for the majority of use cases. |
| `engine.schemaReportingInitialDelayMaxMs` | Integer | On startup, Apollo Server waits a random number of milliseconds between `0` and this value before attempting to register its schema. This helps stagger registration requests when you deploy multiple instances of your server simultaneously. The default value is `10000` (10 seconds). |
...
### Handling conflicting schemas

Whenever you perform a rolling deployment, different active instances of your server might report different schemas to Graph Manager. When this occurs, Graph Manager uses the following algorithm to choose which schema to register:

1. If the specified variant does not yet have an active schema, register the most recently reported schema.
2. If the specified variant _does_ have an active schema and that exact schema has been reported within the last 120 seconds, keep the current active schema.
3. If the specified variant has an active schema and that exact schema has _not_ been reported in the last 120 seconds, register the most recently reported schema.
4. In all other cases, keep the current active schema.

## Manual registration

If you're using an earlier version of Apollo Server or another GraphQL server that doesn't support automatic schema registration, you can register your schema via the [Apollo CLI](https://www.apollographql.com/docs/devtools/cli/).
...
**If you're using [automatic schema reporting](#automatic-registration)** with Apollo Server, registration happens automatically after your server's deployment completes. No additional continuous delivery configuration is required.
