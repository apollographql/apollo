...
## Registering your schema automatically
...
If you use [Apollo Server](https://www.apollographql.com/docs/apollo-server/) version 2.14 or later, you can configure it to report its schema to Graph Manager automatically every time it starts up.

To do so, first obtain a **graph API key**:

<ObtainGraphApiKey />

Then, set this API key as the value of the `APOLLO_KEY` environment variable in your server's environment. Now every time your server starts up, it reports its schema to Graph Manager.

### Registering automatically to a variant

Your server can report its schema to a particular [variant of your graph](#managing-environments-with-variants). Each of your server's environments (staging, production, etc.) should correspond to a different variant.

To set this up, set the `APOLLO_GRAPH_VARIANT` environment variable in each server environment:
...
If you're using [automatic schema reporting](#registering-your-schema-automatically), registration happens automatically each time you deploy your production server. No additional continuous delivery configuration is required.
