...
> Operation safelisting is a **paid feature** that is available as part of Apollo Studio [Team and Enterprise plans](https://www.apollographql.com/plans/).
...
- An Apollo Studio API key.
  - To obtain an API key, visit [Apollo Studio](https://engine.apollographql.com) and create a service. You will need a Studio account with a Team or Enterprise subscription to be able to upload to the operation registry.
...
6. [Start Apollo Server with Apollo Studio enabled](#6-start-apollo-server-with-apollo-graph-manager-enabled)
