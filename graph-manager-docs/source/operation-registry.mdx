...
> **Note:** Operation safelisting is an Apollo Platform feature that is only available to subscribers of the [_Apollo Team_ and _Enterprise_ plans](https://www.apollographql.com/plans/). Visit [this guide](https://www.apollographql.com/docs/intro/platform) to learn more about the Apollo Platform.
...
Operations defined within client applications can be extracted and uploaded to Apollo Graph Manager using the [Apollo CLI](https://www.apollographql.com/docs/devtools/cli/). Apollo Server then fetches a manifest of these operations from Apollo Graph Manager and forbids the execution of any operations that were not in that manifest.
...
- An Apollo Graph Manager API key.
  - To obtain an API key, visit [Apollo Graph Manager](https://engine.apollographql.com) and create a service. You will need a Graph Manager account with a Team or Enterprise subscription to be able to upload to the operation registry.
...
- **Apollo Server** is then configured with a plugin which fetches the manifest from Apollo Graph Manager and enforces safelisting using that manifest.
...
6. [Start Apollo Server with Apollo Graph Manager enabled](#6-start-apollo-server-with-apollo-graph-manager-enabled)
...
> **Note:** If this server's schema has already been registered using `apollo service:push`, you can skip this step. For additional options and details, see the [documentation for the schema registry](/schema-registry/).
...
#### 5.1 Optionally, set the graph variant

Configure the `graphVariant` field to specify which tag to pull operation manifests from.
...
const server = new ApolloServer({
  ...
  plugins: [
    require('apollo-server-plugin-operation-registry')({
      graphVariant: 'overrideTag', // highlight-line
    }),
  ],
});
```
...
- **server**: Set the `graphVariant` field of `apollo-server-plugin-operation-registry` to the targeted graph variant/tag

```js
const server = new ApolloServer({
  plugins: [
    require('apollo-server-plugin-operation-registry')({
      graphVariant: 'prod', // highlight-line
