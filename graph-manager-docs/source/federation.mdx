...
In the rare case where a schema change is _not_ backward compatible with your gateway's query planner, you should update your service registry _before_ you deploy your federated services. In particular, schema changes that affect query planning should be done as separate changes to the registry, prior
