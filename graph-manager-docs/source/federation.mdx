...
In general, when calling `service:push`, our goal should always be to roll out changes in a way that is backwards-compatible to existing clients, in
order to avoid any downtime. Because configuration changes can affect the query planner, it's highly recommended to **call `apollo service:push` after
all replicas of an federated service have deployed**. This way, you can be certain that only valid operations will be executable against the graph, and
that operations will not be able to access fields or types that do not yet exist.

In rare cases where the schema changes cause a backwards incompatible change to _query-planning_, however, it's recommended to update the service registry
**before** deploying federated services. In particular, schema changes that affect query planning should be done as separate changes to the registry, prior
to other modifications. This will help avoid the situation that the query planner generates queries that fail validation in downstream services or violate
resolvers' expectations. Some examples of this include:
...
For example, with an interface that has implementations in multiple services, such as the following:
...
In order to remove the `EmailChannel` implementation of the `Channel` interface, you'll want to perform the following steps:

1. Register a new `email` service that removes the `EmailChannel` implementation of `Channel`
1. Deploy a new Email service that removes the implementation

The first step will cause the query-planner to stop sending along fragments `...on EmailChannel`, which would fail validation if sent
to a service that did not have awareness of that type!

#### Modifying query-planning logic

You'll want to treat migrations of query planning logic similarly to how you may treat database migrations, by carefully considering the effects on the
downstream services as the query planner changes, and planning for "double reading" as appropriate. For example, consider the case of migrating a
`@required` field in an extension from one to another. In this example, we'll consider the `Products` service and the `Reviews` service, like so:
...
If the `nameLowercase` field is to be deprecated and replaced with the `name` field
...
To do this migration in-place, you would perform the following steps:

1. Modify the `Products` service to add the new field (as per usual, first deploy all replicas, then `apollo service:push` the new partial schema)
...
1. Modify the schema of the `Reviews` service in the registry to update query-planning logic to `@require` the new `name` field
1. Deploy a new version of the `Reviews` service with a resolver that only accepts the `name` in its source object

Alternatively, you could perform this operation with an atomic migration at the service level, by modifying the URL of the service. For that, the
steps would be:
...
1. Deploy a new set of `Reviews` replicas to a new URL
1. Register the `Reviews` service with the new URL and the partial schema changes above

With this atomic strategy, the query planner will resolve all outstanding requests to the old service that relied on `nameLowercase` with the old
query-planning configuration, which `@required` the `nameLowercase` field, and all **new** requests would be made to the new service (due to the
