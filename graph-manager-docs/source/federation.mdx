...
In general, when calling `service:push`, you should always roll out changes in a way that is backward compatible with existing clients to avoid downtime. Because configuration changes can affect the query planner, it's highly recommended to **call `apollo service:push` after
all replicas of a federated service have deployed**. This way, you can be certain that only valid operations will be executable against the graph, and
...
to other modifications. This helps avoid the scenario where the query planner generates queries that fail validation in downstream services or violate
...
to a service that isn't aware of that type.
...
1. Modify the Reviews service's schema in the registry so that it @requires the new `name` field.
