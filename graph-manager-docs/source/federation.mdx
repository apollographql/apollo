...
In general, when calling `service:push`, our goal should always be to roll out changes in a way that is backwards-compatible to existing clients, in order to avoid any downtime. Because configuration changes can affect the query planner, it's highly recommended to **call `apollo service:push` after all replicas of an federated service have deployed**. This way, you can be certain that only valid operations will be executable against the graph, and that operations will not be able to access fields or types that do not yet exist.

In rare cases where the schema changes cause a backwards incompatible change to _query-planning_, however, it's recommended to update the service registry **before** deploying federated services. In particular, schema changes that affect query planning should be done as separate changes to the registry, prior to other modifications. This will help avoid the situation that the query planner generates queries that fail validation in downstream services or violate resolvers' expectations. Some examples of this include:
...
You'll want to treat migrations of query planning logic similarly to how you may treat database migrations, by carefully considering the effects on the downstream services as the query planner changes, and planning for "double reading" as appropriate. For example, consider the case of migrating a `@required` field in an extension from one to another. In this example, we'll consider the `Products` service and the `Reviews` service, like so:
