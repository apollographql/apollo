...
In rare cases where the schema changes cause a backwards incompatible change to _query-planning_, however, it's recommended to update the service registry **before** deploying federated services. This will help avoid the situation that the query planner generates queries that fail validation in downstream services. This may occur for one of the following reasons:
