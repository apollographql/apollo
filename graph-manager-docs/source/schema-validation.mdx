...
We recommend defining a separate CI job for each [environment (production, staging, etc.)](#validating-against-multiple-environments) you validate schema changes against. The `apollo service:check` command returns a non-zero exit code when it detects a breaking change, meaning the job will fail when validation fails.
