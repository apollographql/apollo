...
If your data graph uses [Apollo Federation](/federation/#validating-changes-to-the-graph), the schema validation process differs slightly.

For details, see [Validating changes to the graph](/federation/#validating-changes-to-the-graph).

## Using with continuous integration

Schema validation is especially useful when you add it to your continuous integration pipeline (e.g., Jenkins or CircleCI). By doing so, you can detect potential problems automatically and display validation results directly on pull requests.

The `apollo service:check` command returns a non-zero exit code when it detects a breaking change, and it [integrates with GitHub statuses](#integrating-with-github), making it straightforward to use with most CI tools.

### CircleCI example

Here's an example of how to add a schema validation check to CircleCI:
...
> **Note:** If you're using GitHub status checks, we recommend _ignoring_ the exit code of the `apollo service:check` command so your continuous integration can complete without failing early. You can do this by appending `|| echo 'validation failed'` to the command call.
...
You might want to validate your schema changes against multiple environments, such as production, staging, and beta. Each of these environments might have a slightly different schema to support features that are experimental or in active development. In Apollo Graph Manager, these schemas typically belong to [variants](./schema-registry/#managing-environments-with-variants) of a single data graph.
