...
Two semantically identical schemas can appear different to Graph Manager, such as when those schemas list the same set of object fields in a different order. To avoid this scenario, every edge server should **normalize** its schema before sending it to Graph Manager.
