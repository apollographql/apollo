...
This document specifies the protocol that any GraphQL server can implement to enable automatic [schema registration](./schema-registry/) with Apollo Studio.
...
Two semantically identical schemas can appear different to Apollo Studio, such as when those schemas list the same set of object fields in a different order. To avoid this scenario, every edge server should **normalize** its schema before sending it to Studio.
