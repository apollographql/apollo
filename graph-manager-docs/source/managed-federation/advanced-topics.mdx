...
To view the implementing services that make up your graph, you can use both the Apollo CLI and the [Graph Manager UI](https://engine.apollographql.com/).
...
Operation-level statistics will still be collected over the operations sent by the client, and those operations will be validated as part of the `service:check` workflow.
...
In particular, along with [validating overall schema changes against known operations](/schema-checks/), running `apollo service:check` for a federated service will ensure that the overall graph still composes to a valid schema, and will output any violated dependencies if present.
