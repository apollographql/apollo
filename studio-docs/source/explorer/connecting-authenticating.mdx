...
#### Preflight script API
| Function | Type | Description  |
|---|---|---|
| `explorer.environment.get` | <InlineCode>(key: string) => Readonly<JSONValue \| undefined></InlineCode> | Used to retrieve the current value of an environment variable by it's key |
| `explorer.environment.set` | `(key: string, value: JSONValue) => void` | Used to set a new value for an environment variable by it's key |
| `explorer.fetch` | <InlineCode>(href: string, options?: { method?: string, body?: string \| null, headers?: Record<string, string> }) => Promise<{ code: number, body: string, json: () => any }></InlineCode> | Used to make network requests to external services |
| `explorer.prompt` | <InlineCode>(msg: string, defaultResponse?: string) => Promise<string \| null></InlineCode> | Used to prompt the user for input and return the value in a promise. If the user cancels the prompt `null` is returned |
