...
* [The Explorer](./explorer/), including support for [operation collections](./operation-collections)
* [Schema checks](../schema-checks/)
* Schema reference
* Schema diffs against registered graphs (for logged-in users)
...
The Sandbox version of [the Explorer](./explorer/) automatically attempts to connect to a GraphQL server running at `http://localhost:4000`. You can use the box at the top of the Explorer to change this URL to any local or remote GraphQL endpoint that's accessible by your browser:
