...
# Tagged type
# You can tag an object type which will  in all its fields tagged All fields part of the "Product" object inherit the "partner" tag so we can avoid tagging them explicitly
type Product @key(fields: "upc") @tag(name: "partner") {
  upc: ID!
  name: String!
  description: String!
  # Tagged fields
  # The internalId, and percentageMatch fields would also inherit the "partner" tag, so they need
  # the "private" and "experimental" tag (and an exclude filter) to keep being inaccessible
...
* You can apply tags to the following in your schema:
    ...
    * The tags on an object or interface are inherited by all of its fields (as shown above)
