...
You can embed The Explorer into your websites, so folks can query your public variants. This example allows you to make queries to our Acephei example graph.

// TODO: (MAYA) add the gatsby theme embedded explorer here <EmbeddedExplorer/>

You will need to configure the parent app that embeds The Explorer to accept post messaged network requests from The Explorer, and to post message the responses back to The Explorer. You can do so with the following code snippet. You will need to replace the url to fetch from with your graphs url, and the graph ref in the EMBEDDABLE_EXPLORER_URL with your graph ref.
