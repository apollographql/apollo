...
Turning on the **Inlined traces** feature in the Explorer's settings will tell the Explorer to forward a special header to your server to request that it returns a protobuf with tracing information along with its response.
