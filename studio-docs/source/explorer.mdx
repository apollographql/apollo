...
The bottom panel of the Explorer editor includes an Environment Variables tab. Here, you can provide sensitive information that you can then include in GraphQL variables, header values, or [preflight scripts](#preflight-scripts-beta). This enables you to share operations (including headers and variable values) with other team members _without_ exposing sensitive data.
...
You can then include that environment variable in header values and GraphQL variables with the following syntax:
