...
```jsx
<iframe src="https://explorer.embed.apollographql.com/?graphRef=my-graph-id@my-variant&sendRequestsFrom=embed" />
```

As shown, the `iframe`'s `src` URL requires the following query parameters:

* `graphRef` - This indicates which [public variant's](./org/graphs/#public-variants) schema the Explorer should download from Apollo Studio (e.g., `my-graph-id@my-variant`).
...
There are two supported methods for handling this: [adding a CORS domain](#add-a-cors-domain-recommended) or [routing requests through the parent page](#route-requests-through-the-parent-page-advanced).
...
If `embed`, the embedded Explorer transmits operations directly to your GraphQL server from the origin `https://explorer.embed.apollographql.com`.
...
If [`shouldPersistState`](#shouldpersiststate) is `true`, the editor is _also_ populated with user-defined operations from `localStorage`.
...
If `true`, the embedded Explorer uses `localStorage` to persist user-defined operations between sessions. These operations are automatically populated in the Explorer on page load.
...
If you're [embedding the Explorer](#embedding-the-explorer), it's possible that your GraphQL server can't accept requests from `https://explorer.embed.apollographql.com` due to organizational policies. In this case, your webpage can use the `postMessage` API to act as an intermediary between the embedded Explorer and your server:
