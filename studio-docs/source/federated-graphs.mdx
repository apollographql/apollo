...
A **federated graph** is a graph that’s composed of multiple individual **subgraphs**:
...
- If your subgraph doesn’t already have a `schema` object, you need to add one to use the `@contact` directive. The `schema` object cannot be empty, so you need to include at least one field in it (most commonly `query: Query`). If you do provide the `Query` object like this, it needs to include at least one field in your subgraph’s schema. If your subgraph’s `Query` object is empty, you need to add a dummy field for the SDL to be valid:
```graphql:title=schema.graphql
@contact(...)
schema {
  query: Query
