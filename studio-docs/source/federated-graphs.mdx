...
- download a copy of your subgraph’s SDL for working locally
...
Once a subgraph is using the `@contact` directive, its contact information will be included with the SDL’s metadata at the top of the SDL file.

**[📸screenshot: SDL metadata with contact info]**

You’ll also see a contact card when you hover over the subgraph column of schema objects in the Schema reference, so you can quickly see who is responsible for that object, and where to contact them for questions or support.

**[📸screenshot: query field row with subgraph and contact tooltip on hover]**

### Adding the @contact directive to your subgraph

The `@contact` directive can be added to any subgraph directly above the `schema` object in your SDL. If your subgraph does not have a schema object, you’ll need to add one to use the `@contact` directive, and it cannot be empty, so you will need to add the `query` field.

Example:

```graphql:title=subgraph_example.graphql
directive @contact(
...
Available fields:
- `name: String!` - used to identify the person or team responsible for the subgraph. This is the only required field for using the `@contact` directive.
- `url: String` - used to provide a URL where the subgraph maintainers can be reached for questions or discussion. For example, you might link to a repository, chat channel, forum, or email address.
- `description: String` - used to provide helpful details about working with this subgraph, or contacting its maintainers. This field also supports markdown links.
...
- If your subgraph doesn’t already have a `schema` object, you will have to include one to use the `@contact` directive. The `schema` object cannot be empty, so you’ll need to include field on it, like `query: Query`. And if you use the `Query` field, it will need to reference a type within the subgraph, so if you do not have one that can be referenced, you’ll need to include a fake one for the SDL to be valid. It’s kind of a hack for now. :-)
```graphql:title=subgraph_example.graphql
@contact(...)
schema {
  query: Query
...
Studio’s Explorer also has improved support for working with Federated graphs. If you’d like to preview how your server resolves the data for your operations, you can now see the Query plan in the Operation results pane.
