...
Note that every graph in Studio has a **globally unique name**. We recommend that you prefix your graph names with the name of your organization to avoid naming collisions.
...
To distinguish between instances of the same graph running in different environments (such as staging and production), you can define **variants** for a graph. Each variant has its own schema, along with its own change history, metrics, and operation registry.
