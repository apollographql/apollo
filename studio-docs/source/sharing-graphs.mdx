...
You embed the Explorer in an `iframe` that points to `https://explorer.embed.apollographql.com`. You add a `graphRef` query parameter to this URL to indicate which [public variant](#public-graphs) the Explorer should use:
...
If you're using the `postMessage` API with your embedded Explorer, every message posted this way has an `event.data.name` which denotes the type of message.
...
Posted by your page to notify the `iframe` of an operation to fill in the Explorer. You might want to do this on load, so folks can see a default operation for your graph.
...
Posted by the `iframe` to notify your page that the Explorer is ready for you to send a `SetOperation` request.
