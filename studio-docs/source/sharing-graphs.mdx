...
```jsx
<iframe src="https://explorer.embed.apollographql.com/?graphRef=my-graph@my-variant&postMessageOperations=false" />
```

As shown, the `iframe`'s `src` URL requires two query parameters:

* `graphRef`, which indicates which [public variant's](#public-graph-access) schema the Explorer should download from Apollo Studio (e.g., `my-graph@my-variant`)
* `postMessageOperations`, which indicates whether the `iframe` is responsible for sending its own network requests to your GraphQL server (this depends on how you [handle CORS](#handling-cors))

#### Handling CORS

The embedded Explorer can transmit GraphQL operations directly to your GraphQL server. Because the Explorer is embedded in an `iframe`, the origin of its network requests is `https://explorer.embed.apollographql.com`.
...
* Use the `postMessage` API to send requests from your webpage's domain _instead of_ the `iframe` (advanced)

**If you're sending requests from the `iframe`**, set the `iframe` URL's `postMessageOperations` parameter to `false`. You can now add the `iframe` to any webpage and set its dimensions as you like. Also check out [additional supported options](#options).

**If you're sending requests from your webpage's domain**, set the `iframe` URL's `postMessageOperations` parameter to `true`. Then proceed to the additional setup in [Using `postMessage`](#using-postmessage-advanced).
...
For details, see [Using `postMessage`](#using-postmessage-advanced).
