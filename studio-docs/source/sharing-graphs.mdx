...
**Each variant has its own README.** If you edit one variant's README, those edits are _not_ applied to other variants. Among other uses, this enables you to provide different information to external consumers in a [public variant](#making-a-graph-public).

### Supported Markdown

The README supports all [basic Markdown syntax](https://www.markdownguide.org/cheat-sheet/#basic-syntax). Most [extended syntax](https://www.markdownguide.org/cheat-sheet/#extended-syntax) is _not_ fully supported, except for code blocks.
...
You can enable public access to individual [variants](./org/graphs/#managing-variants) of your graph. If you do, **anyone with that variant's public link** can view the following pages for that variant in Studio:
...
If you have a [public variant](#public-graph-access) of your graph, you can embed the [Apollo Studio Explorer](./explorer/) in a webpage that you can then provide to your graph's consumers. This enables those consumers to test out operations from your own website.
...
You embed the Explorer in an `iframe` that points to `https://explorer.embed.apollographql.com`. You add a `graphRef` query parameter to this URL to indicate which [public variant](#public-graph-access) the Explorer should use:

```jsx
<iframe src="https://explorer.embed.apollographql.com/?graphRef=my-graph@my-variant" />
```

The embedded Explorer uses this `graphRef` to fetch your public variant's schema and its GraphQL endpoint URL from Apollo Studio.

#### Handling CORS

By default, the embedded Explorer sends user-executed operations directly to your variant's GraphQL endpoint. Because the Explorer is embedded in an `iframe`, the origin of those network requests is `https://explorer.embed.apollographql.com`.

Because of this, you need to do one of the following:

* Update your GraphQL server's CORS configuration (such as with the [`cors` `ApolloServer` option](https://www.apollographql.com/docs/apollo-server/api/apollo-server/#cors)) to accept requests from `https://explorer.embed.apollographql.com`
    * This method is more straightforward if your organization allows it.
* Use the `postMessage` API to send requests from your webpage _instead of_ the `iframe` (advanced)

**If you update your server's CORS configuration,** you're all set! You can now add an embedded Explorer `iframe` to any webpage and set its dimensions as you like. Also check out [additional supported URL parameters](#options).

**If you need requests to originate from your webpage**, additional setup is required. See [Using `postMessage`](#using-postmessage).
...
```jsx
<iframe src="https://explorer.embed.apollographql.com/?graphRef=my-graph@my-variant&theme=light" />
```
...
**Required.** The graph ref for the [public variant](#public-graph-access) you want to use the embedded Explorer with. Has the format `graph-name@variant-name`.
...
URI encoded operation to populate in the explorer editor on load.

</td>
<td>

If you omit this, the explorer loads an example query from your schema on first load, and then saves the users last operation in local storage for the future.

Example: `?document=${window.encodeURIComponent(`
query ExampleQuery {
  id
}`)}`
...
URI encoded variables to populate in the explorer editor on load.

</td>
<td>

You can set both `variables` and `document` to allow your users to run a query with example variables on load.

Example:  `?variables=${window.encodeURIComponent(
      JSON.stringify({
...
URI encoded headers as JSON to populate in the explorer editor on load.

</td>
<td>

You can set `headers` to allow your users to run a query with example headers on load.

Example: `?headers=${window.encodeURIComponent(
      JSON.stringify({
        ExampleHeader: "hello"
      })
    )}`
...
A schema entity to be pre-populated in explorer spotlight search on load. 

</td>
<td>

Example: `?searchQuery=Query.id` will open the spotlight search with `Query.id` in the search bar.
...
> Before you proceed, make sure you have a [public variant](#public-graph-access) of your graph in Apollo Studio.
...
Notice above that we append some query parameters to our `iframe`'s URL. The most important of these is `graphRef`. This tells the Explorer which [public variant's](#public-graph-access) schema to download to populate the Documentation panel and enable symbol completion.
...
Posted by your page to provide the `iframe` with an operation string to populate the Explorer's editor. You might want to use this to populate a default operation on page load.
