...
* `postMessageOperations`, which indicates whether the `iframe` is responsible for sending network requests directly to your public variant's GraphQL endpoint (you set this according to how you [handle CORS](#handling-cors))
...
**If you're sending requests from the `iframe`**, set the `iframe` URL's `postMessageOperations` parameter to `false`. Also make sure you've set your public variant's endpoint URL in Apollo Studio.

You can now add the `iframe` to any webpage and set its dimensions as you like. Also check out [additional supported options](#options).
