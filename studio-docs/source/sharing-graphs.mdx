...
Notice that we append some query parameters to our `iframe`'s URL, most importantly `graphRef`. This tells the Explorer which [public variant's](#public-graphs) schema to download to populate the Docuentation panel and enable symbol completion.

All supported query parameters are listed in [Options](#options).
...
If you omit this option, the Explorer still loads, but it can't provide any useful information about the graph the user is querying against.
