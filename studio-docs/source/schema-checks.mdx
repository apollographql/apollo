...
2. Enable metrics reporting to Apollo Studio from your production graph. Studio uses these metrics to help determine when a type or field can be modified or removed safely.
    * [Federated setup](./setup-analytics/)
    * [Non-federated setup](./setup-analytics/)
...
> **Important:** If your graph is [federated](https://www.apollographql.com/docs/federation/), instead use [`rover subgraph check`](https://www.apollographql.com/docs/rover/subgraphs/#checking-subgraph-schema-changes). This command requires one additional option, the `--name` of the subgraph you're checking against.
...
In addition to the arguments required by `graph check` , `subgraph check` requires the `--name` of the particular subgraph you're checking against.

When you run `subgraph check`, Apollo Studio performs operation checks as usual, along with **composition checks**: Studio verifies that the changes you make to a subgraph schema are compatible with your _other_ subgraph schemas, enabling them to compose into a _supergraph_ schema ([learn more](https://www.apollographql.com/docs/federation/quickstart/#federation-concepts)).
...
You can click on a composition check to view its result. If composition succeeded, you can view the composed supergraph schema.

The `rover subgraph check` command returns a nonzero result if any associated operation check _or_ composition check fails.
...
If you're using GitHub, you can install the [Apollo Studio GitHub app](https://github.com/apps/apollo-studio). This app enables Apollo Studio to send a webhook back to your GitHub project on each call to `rover graph check`, providing built-in pass/fail status checks on your pull requests:
