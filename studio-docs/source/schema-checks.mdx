...
If you're using GitHub, you can install the [Apollo Engine GitHub app](https://github.com/apps/apollo-studio). This app enables Apollo Studio to send a webhook back to your GitHub project on each call to `apollo service:check`, providing built-in pass/fail status checks on your pull requests:
...
```
You can also set these threshold customizations more permanently using [Check Configurations](#check-configurations) in [Apollo Studio](https://studio.apollographql.com/service/engine/checks?...).
