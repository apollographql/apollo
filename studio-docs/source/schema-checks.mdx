...
> This article was recently updated to use the new Rover CLI instead of the Apollo CLI. If you are already using the Apollo CLI for schema checks, [see this article](./schema-checks-apollo-cli/).
...
> **Important:** If your graph is [federated](https://www.apollographql.com/docs/federation/), instead use [`rover subgraph check`](https://www.apollographql.com/docs/rover/subgraphs/#checking-subgraph-schema-changes). This command requires one additional option, the `--name` of the subgraph you're performing checks for.
