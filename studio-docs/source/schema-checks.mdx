...
3. Studio returns the diff, along with a list of any breaking changes found.
...
* **Mark the operation as unsupported.** In this case, schema checks will completely ignore the operation when checking _all_ changes in any future execution.
  ...
  * You can view all ignored operations for your graph on its [check configuration page](./check-configurations/).
