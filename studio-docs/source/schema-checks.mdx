...
### Ignoring operations 
You might also want to ignore operations from unsupported clients/client versions. In these cases, you can ignore the operation entirely. Similar to approving behavior changes for an operation, on the next check, the affected operation will appear under the "Ignored Operations" section and will not cause subsequent checks to fail. You can ignore an operation by clicking on "Override" and then choosing "Mark operation as unsupported" to achieve this.
