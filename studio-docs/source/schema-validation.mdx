...
Schema checks is a **paid feature** that is available as part of Apollo Studio [Team and Enterprise plans](https://www.apollographql.com/plans/).
...
> **Note:** Because breaking changes are detected by analyzing recent operations, your GraphQL server _must_ be [pushing analytics to Apollo Studio](/setup-analytics/) for schema checks to work. **If there are no operation metrics to compare against, all schema changes are labeled with `PASS`.**
...
> **Note:** If you've [installed schema checks checks on your GitHub PRs](#integrating-with-github), the "Details" link in your GitHub checks will take you to the same details link in this output.

### Validating a federated schema

If your data graph uses [Apollo Federation](https://www.apollographql.com/docs/apollo-server/federation/introduction/), the schema checks process differs slightly. For details, see [Validating changes to a federated graph](/managed-federation/advanced-topics/#validating-changes-to-a-federated-graph).

## Using with continuous integration

Schema checks are especially useful when you add it to your continuous integration pipeline (such as Jenkins or CircleCI). By doing so, you can obtain check results and display them directly on your team's pull requests.
...
If you're using GitHub Enterprise, Bitbucket, or another version control service, we recommend setting up your CI tool to post a comment on each pull request with the results of schema cjecls. By surfacing schema diffs and breaking changes directly in your PR, you can avoid having to search your CI logs to check why the check failed.

The `apollo service:check` supports a `--markdown` flag, which outputs the results of schema checks in markdown format. You can pipe this output directly into a comment to your source control tool, like in [this example of posting a comment with the results of a schema check to GitHub](https://gist.github.com/daniman/e53d0589d18b778878bd8ef32d2e793c).
...
You can customize the logic that schema checks use to determine which operations to check schema changes against.
...
You can also customize the behavior of schema checks by specifying **threshold values** to the `service:check` command. These values tell schema checks to ignore all historical operations that do not exceed the threshold(s) you specify.
...
> **Note:** In some cases, in-place updates _are_ compatible with affected clients at runtime (such as a type rename or a conversion from an object to an interface that uses the same fields). However, your schema check still marks these as breaking changes, because it does not have enough information to ensure that they are safe.
