...
The `useMutation` hook is another important building block in an Apollo app. It uses React's [Hooks API](https://reactjs.org/docs/hooks-intro.html) to provide a function to execute a GraphQL mutation. Additionally, it tracks the loading, completion, and error state of that mutation.
...
Sometimes, we need to access the `ApolloClient` instance to directly call a method that isn't exposed by the `@apollo/client` helper components. The `useApolloClient` hook can help us access the client.
...
const client: ApolloClient<NormalizedCacheObject> = new ApolloClient({
  cache,
  uri: 'http://localhost:4000/graphql', // preserve-line
  headers: {
...
Using `ApolloClient`'s `headers` option allows us to read the token from `localStorage` and attach it to the request's headers each time a GraphQL operation is made.
