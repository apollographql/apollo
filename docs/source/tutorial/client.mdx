...
Now, our dependencies are installed.
...
Add this code to the bottom of `index.tsx`:
...
To connect Apollo Client to React, we'll wrap our app in an `ApolloProvider` component and pass our client to its `client` prop. The `ApolloProvider` component is similar to React’s context provider. It wraps your React app and places the client on the context, which allows you to access it from anywhere in your component tree.

Open `src/index.tsx` and add the following lines of code:

<MultiCodeBlock>

```tsx{6,8-11,15-21}:title=src/index.tsx
import {
  gql,
  ApolloClient,
  InMemoryCache,
...
```jsx{5,7-10,14-20}:title=src/index.jsx
import {
  gql,
  ApolloClient,
  InMemoryCache,
