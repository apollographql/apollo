...
Apollo provides an extension for Visual Studio Code that includes loads of helpful features, such as syntax highlighting, GraphQL operation autocomplete, and in-line performance estimates.
...
To connect Apollo Client to React, we wrap our app in the `ApolloProvider` component exported from the `@apollo/client` package. We pass the component our `ApolloClient` instance via the `client` prop.
