...
First, make a copy of the `.env.example` file located in `client/` and call it `.env`. Add your Graph Manager API key that you already created in [step #5](https://www.apollographql.com/docs/tutorial/production/#get-a-graph-manager-api-key) to the file:
