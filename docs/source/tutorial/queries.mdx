...
The schema field that our server paginates is the list of `launches`. Let's add a `merge` function for this field to the cache's configuration, like so:
...
> For a list of all support fetch policies, see [Configuring fetch logic](https://www.apollographql.com/docs/react/data/queries/#configuring-fetch-logic).
