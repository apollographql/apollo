...
Copy the code below and add it before the closing `</Fragment>` tag in the render prop function we added in the previous step.
...
```tsx{6,10}:title=src/pages/launches.tsx
const GET_LAUNCHES = gql`
  query launchList($after: String) {
    launches(after: $after) {
      cursor
