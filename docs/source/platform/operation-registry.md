...
4. Enable demand control by adding the operation registry to Apollo Server.
    ...
    To enable the operation registry within Apollo Server, it's necessary to install and enable the `apollo-server-plugin-operation-registry` plugin and ensure Apollo Server is configurated to communicate with Apollo Engine.
