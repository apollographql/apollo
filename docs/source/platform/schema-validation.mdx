...
But wait... What exactly is composition? 🤔 In order to have different implementing services contribute to the same graph, there needs to be a way to combine each implementing service's partial schemas into one large data graph. Engine's composition algorithm does exactly this.

Here's how schema validation works for federated and non-federated services:

1. You run `apollo service:check` locally or in CI. For federated services run the same command but add `--serviceName=ExampleService --endpoint=http://loocalhost:4000/graphql` to the end. The proposed schema/partial schema (if using federation) is sent to Engine's schema registry.
1. If you're using federation, Engine's schema registry will attempt to compose the partial schema with the current schema. If composition fails then we do not continue with validation and exit early.
1. Engine creates a diff between the local schema and the most recently published schema in the registry. For federation this step happens after composition passes.
