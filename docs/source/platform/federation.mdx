...
While federation empowers teams to scale GraphQL much faster than before, it also introduces complexity that comes along with running a distributed system. Teams need federation-aware tooling in order to help them coordinate updating the graph whenever an underlying service changes. Apollo's **graph manager** is a turnkey solution to this problem that includes federated service checks, managed service deployments, and federation-aware analytics out of the box for teams on a paid plan.
...
1. [Configure Apollo Server](#connecting-apollo-server-to-the-registry) to connect to the registry.
...
1. [Integrate your federated graph](#integrate-with-your-deployment-pipeline) into your deployment pipeline.
...
## Connecting Apollo Server to the registry
...
> NOTE: The managed configuration will default to using the `'current'` variant. To use a different variant, set the `ENGINE_GRAPH_VARIANT` environment variable.
...
Traces will be reported in the shape of the query plan, with each unique fetch to an implementing service reporting timing and error data.
...
As an engineer working on an implementing service, you might wonder how to guarantee that the changes you're making to your service are safe changes for the graph as a whole. When rolling out changes to an implementing service, we recommend the following worflow:
...
1. Deploy changes to the implementing service in your infrastructure
1. Let all replicas finish deploying
1. Call `apollo service:push` to update the implementing service
...
Every time that `apollo service:push` is called for a federated service, it not only registers the implementing service to the graph, but it also updates the managed configuration files that the gateway has access to. Because the graph is dynamically changing, it's possible for composition errors to occur for a `service:push` even after a `service:check` has succeeded if other implementing services changed in the interim. For this reason, updating an implementing service will re-trigger composition in the cloud, ensuring that the implementing services still compose to form a complete graph before provisioning the managed configuration. The workflow behind the scenes can be summed up as follows:

1. The partial schema is uploaded to a secure location and indexed
1. The implementing service is updated in the registry to point to the partial schema
1. All implementing services are composed in the cloud to produce a new complete schema
1. If composition fails, the command exits and emits errors
1. If composition succeeds, the top-level managed configuration file is updated in-place to point to the updated set of implementing services
...
1. On update, the gateway downloads configuration for each implementing service in parallel
...
1. Roll over the `prod-canary` graph, containing one gateway container, using
   ...
   ```
>Note: If composition fails due to intermediate changes to the canary graph, new configuration will not be rolled out
