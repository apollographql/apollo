...
- For a [federated](https://www.apollographql.com/docs/apollo-server/federation/introduction/) graph, Apollo supports registering individual services that make up your graph and associating them with partial schemas. This separates ownership across implementing services and allows you to track the composition of your graph from its implementing services. See below for details on registering federated graphs. 
...
To push a single implementing service to Apollo, run `apollo service:push` with the `serviceName` and `endpoint` tags. Note that running `apollo service:push` on any implementing service also registers the overall schema of the graph and writes files to GCS.
...
id      schema                        tag
──────  ────────────────────────────  ───────
d47a8b  service-list-federation-demo  current
```
...
> Note: Currently, you can only see the history of your graph's composed schema, not of partial schemas.
