...
- **Apollo Server** is a JavaScript GraphQL server for defining a
  ...
  _schema_. Typically Apollo Server is extensible: plugins can hook in to each stage of the
...
- **Apollo Client** is a sophisticated GraphQL client that
  ...
  exceptionally elegant approach to state managment by _extending_ the
